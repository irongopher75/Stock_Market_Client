    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: '#000', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '0', overflowY: 'auto' }}>
            {loading && data.length === 0 ? (
                <div style={{ padding: '20px', color: '#888', fontSize: '11px', ...mono }}>FETCHING FX RATES...</div>
            ) : error ? (
                renderErrorState()
            ) : data.length === 0 ? (
                <div style={{ padding: '20px', color: '#444', fontSize: '11px', ...mono, textAlign: 'center' }}>NO LIVE FX DATA AVAILABLE</div>
            ) : (
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px', ...mono, textAlign: 'left' }}>
                    <thead>
                        <tr style={{ background: '#111', color: '#666', borderBottom: '1px solid #1A1A1A' }}>
                            <th style={{ padding: '8px 12px', fontWeight: 'normal' }}>PAIR</th>
                            <th style={{ padding: '8px 12px', fontWeight: 'normal', textAlign: 'right' }}>RATE</th>
